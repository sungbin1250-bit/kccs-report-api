...
<title>KCCS Backend Safe Test v4</title>
<style>
...
$("run").addEventListener("click", async () => {
  ...
  try {
    ...
    const finalize = await post("/api/kccs/finalize-v3", secret, {
      date: TEST_DATE,
      dryRun: true,
      samsungReturn: 1.0,
      skHynixReturn: -0.5,
    ...
    if (cleanup.status !== "TEST_SIGNAL_DELETED") {
      throw Object.assign(
        new Error("TEST_SIGNAL_DELETED 응답이 아닙니다."),
        { data: cleanup }
      );
    }
    ...
    mark(
      "s3",
      "ok",
      `3. 테스트 신호 자동 삭제 — 성공 (${cleanup.deletedCount ?? 0}건)`
    );
    ...
    try {
      await post("/api/kccs/signal-v3", secret, {
        action: "DELETE_TEST",
        date: TEST_DATE
      });
    } catch (_) {}
