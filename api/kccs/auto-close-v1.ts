...
const SERVICE_VERSION = "kccs-auto-close-v2-shared-market-snapshot";
      ...
      return Response.json(
        {
          ...
          flow: [
            "Yahoo daily/intraday market snapshot",
            "KCCS auto model",
    ...
    try {
      finalized = await postJson(
        ...
        cronSecret,
        {
          ...
          dryRun: false,
          force: body.force === true,
          samsungReturn: n(model?.market?.samsungReturn),
          skHynixReturn: n(model?.market?.skHynixReturn),
          marketSource:
            model?.market?.source ||
            model?.market?.marketSource ||
            "KCCS model market snapshot",
        }
      );
    } catch (error) {
      return Response.json(
